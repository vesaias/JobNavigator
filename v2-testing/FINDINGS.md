...
**Status** fixed: systematic pass done (73 one-line line-heights, `stage3/F-009-linheights.md`); after rebuild `lh_scan3`: bordered elements on a fraction = 0 on every route; residual unbordered fractional blocks: Feed 14 (title line-height 1.15 inside a fixed 20 px box, deliberately left), Applications 98 (offset only, no fractional element, cosmetic), Résumé editor 5, Settings 3, Stats 44 (Recharts SVG). Searches/Companies/Résumés shelf/Cover Letters/Persona = 0. Closed as P4 residue.
...
**Status** fixed + verified: with no filters and zero rows the list shows "No open roles yet — Jobs arrive from Searches and Companies…" with working links; "No jobs match." remains for filtered misses
