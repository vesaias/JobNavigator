...
- [ ] **R4 text candidates** (Opus) → `round2/text-candidates.md`. Every user-facing string in `frontend/src/v2/*.jsx` longer than ~6 words (toasts, empty states, helper/sub-lines, tooltips, modal copy, settings help/info) listed as `file:line | current text | flag` where flag ∈ {mannered, metaphor, hedge, long, fine}. No rewrites.
- [ ] **R4b text suggestions** (Fable) → `round2/text-suggestions.md`. Plain rewrites for every flagged line; prompt: "Avoid mannered prose, say things plainly. No metaphors or figures of speech." Suggestions only — nothing applied.
