...
Twelve cover-all decisions, accepted and applied to `frontend/src/v2/*.jsx`. Rows in the
per-screen tables that these settle outright have been removed (listed under
"Removed as settled" at the end); rows they settle only in part now carry the new
wording in their **current** column.

1. **sweep** (scrape sweep, last sweep, next sweep) → **scrape run** ("last scrape run", "next scheduled run") — V2App, Stats, Searches, Settings
2. **land / lands / landed** → **appear / are applied / is recorded** — Companies, JobFeed, Settings, Searches, Resumes, ResumeEditor
3. **live in / lives under** → **are in / is under** — Searches, Settings, Resumes, Stats
4. **draw from / drawn from / draw on** → **use / is based on / taken from** — Persona, Companies, CoverLetterEditor
5. **funnel** — kept as the chart name in Stats; dropped from hints elsewhere ("kept in Stats") — Applications, Settings
6. **tracer / tracers / tracer links** → **tracked links**; the first use on a screen glosses it as "tracked links — short links that record when a recruiter opens them" — Résumés, Cover Letters, Feed, Settings
7. **dry run** → **preview run — saves nothing** — Searches, Companies
8. **JD** → **job description** — ResumeEditor
9. **LCA / LCAs** → **H-1B filings** — Feed (the code already read "H-1B filings"; no edit was needed)
10. **prep handover / prep bundle / the bundle / handover bundle** → one name: **prep handover** — Applications, Settings
11. **"The Feed"** page title → **Jobs**, matching the rail — JobFeed
12. **quick-scored** → **Light** ("This report was scored at Light depth") — JobFeed

Deliberately **not** decided, still open for evaluation: **slug → id** (Settings, Companies)
and **pipeline → the app / all runs** (ResumeEditor, Stats). Their rows stay in the tables below.
...
| V2App.jsx:95 | No company or search is failing, but the last scrape run failed · ${lastSweep}. Click → Stats · Run history. | All companies and searches are healthy, but the last scrape run failed ${lastSweep}. Click to open Run history. |
| V2App.jsx:96 | All companies and searches healthy · last scrape run ${lastSweep}. Click → Stats · Run history. | All companies and searches healthy · last scrape run ${lastSweep}. Click to open Run history. |

## 2. Résumés (shelf + editor + shared sections)

| file:line | current | suggested |
|---|---|---|
| Resumes.jsx:153 | ${n} bases · ${m} tailored copies are under their jobs · ${k} archived | ${n} bases · ${m} tailored copies, listed under their jobs · ${k} archived |
...
| Searches.jsx:80 | Personalized AI recommendations from your Jobright.ai account. A search term switches it to search mode; credentials are in Settings. | Recommendations from your Jobright.ai account. Enter a search term to search instead. Credentials are in Settings › Accounts. |
...
| Searches.jsx:550 | running now — results appear in the Job Feed as they arrive… | running now. Results appear in the Job Feed as they are found. |
...
| Searches.jsx:532 | Runs on the next scheduled run once created | Runs at the next scheduled scrape after you create it |
| Searches.jsx:582 | Pause — leaves the schedule, config is kept | Pause. Removed from the schedule, settings kept. |
| Searches.jsx:599 | Preview run — shows results and per-job filter reasons, saves nothing | Preview run. Shows results and why each job was kept or filtered. Saves nothing. |
...
| Settings.jsx:317 | Fires only when the scoring call errors or hits a rate limit; one retry, then the job is left unscored for the next scrape run. Pick a cheap, reliable model from a different provider than the Primary so one outage can't take both down. | Used only when the scoring call fails or is rate-limited. One retry, then the job stays unscored until the next run. Choose a cheap model from a different provider than the primary. |
...
| Settings.jsx:376 | Counts from the last activity on the application (stage change, email, note). Auto-rejected applications keep their history and stay in Stats — nothing is deleted. | Counted from the last activity on the application (stage change, email, note). Auto-rejected applications keep their history and stay in Stats. |
| Settings.jsx:417 | Each application gets its own short link per document link. When a recruiter opens one, the hit is recorded in Stats against that application. | Each application gets its own short link for every document link. When a recruiter opens one, it is recorded in Stats for that application. |
...
Eighteen rows left the tables above because §0 settled them outright — the flagged text no
longer exists in `frontend/src/v2/*.jsx` and the row proposed nothing beyond the decision.
Listed here so nothing is lost. The parenthesis is the §0 decision that settled the row.

| removed row | was | now reads |
|---|---|---|
| V2App.jsx:91 (1) | no sweep recorded yet | no scrape run recorded yet |
| Stats.jsx:382 (1) | Last sweep ${failed}${ago} | Last scrape run ${failed}${ago} |
| Companies.jsx:502 (2) | …its career page is scraped and the jobs land in the Feed. | …the jobs appear in the Feed. |
| JobFeed.jsx:1169 (2) | Instant · no LLM cost · lands in Résumés | Instant · no LLM cost · appears in Résumés |
| Searches.jsx:275 (3) | Credentials live in Settings › Accounts | Credentials are in Settings › Accounts |
| Settings.jsx:391 (3) | Telegram bot · digest schedule lives under Scheduler | Telegram bot · digest schedule is under Scheduler |
| Persona.jsx:344 (4) | Tailored résumés draw from whatever you add here. | Tailored résumés use whatever you add here. |
| Applications.jsx:41 (5) | Closed — kept for the Stats funnel | Closed — kept in Stats |
| Stats.jsx:430 (5) | Application funnel | unchanged — the decision is to keep it (see §11) |
| ResumeEditor.jsx:631 (6) | Exact copy of the base, with its own tracer links | Exact copy of the base, with its own tracked links |
| JobFeed.jsx:895 (6) | Copy résumé with tracers | Copy résumé with tracked links |
| Companies.jsx:466 (7) | Dry run — shows what would be kept, writes nothing | Preview run — shows what would be kept, saves nothing |
| JobFeed.jsx:609 (9) | ${verdict} · ${n} LCAs / ${verdict} · no LCA records | already read "H-1B filings" in the code — no edit needed |
| Applications.jsx:493 (10) | Generate prep handover for AI | unchanged — "prep handover" is the accepted single name |
| Applications.jsx:234,235 (10) | Could not build the prep bundle | Could not build the prep handover |
| Applications.jsx:583 (10) | Building the bundle… | Building the prep handover… |
| JobFeed.jsx:633 (11) | The Feed | Jobs |
| JobFeed.jsx:1040 (12) | This report was quick-scored — rescore at full depth… | This report was scored at Light depth — rescore at Full depth… |

Twenty-two further rows were kept but had their **current** column refreshed to the applied
wording, because they also propose a rewrite for length or clarity that is still open:
V2App:95, V2App:96, Resumes:153, Resumes:195, ResumeSections:220 / CoverLetterEditor:390,
ResumeEditor:730, ResumeEditor:783, ResumeEditor:489, ResumeEditor:758, CoverLetterEditor:525,
Stats:566, Searches:80, Searches:550, Searches:532, Searches:599, Companies:537 (filings),
Settings:317, Settings:376, Settings:417, Settings:357, Settings:358, Settings:359.
