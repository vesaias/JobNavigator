...
### R4-T2B-01 01· P2 · Settings' narrow-layout ResizeObserver silently fails to attach on ~1 load in 7, leaving the controls clipped at ≤1150 px
...
**Status**: fixed — `Settings.jsx:208-231`, `:558`. The `ResizeObserver` is attached from a **callback ref** on the scroller instead of an effect keyed on `[!!S]`, so it runs on the commit that actually mounts the node and cannot be missed: the old effect fired on the first of the two renders `setS`/`setDone` can produce, found `scrollRef.current === null` while the component was still returning the `!ready` placeholder, hit its guard, and never re-ran because `!!S` never changed again. The ref callback also disconnects the previous observer, and a single unmount effect disconnects the last one. Worth re-running the reporter's own 20-load loop at 1024x820 against the rebuilt bundle to confirm 20/20 narrow.

### R4-T2B-02 02· P2 · Reloading during a cover-letter regenerate loses every trace of the run, and the letter then rewrites itself with no notice
...
**Status**: fixed — `CoverLetterEditor.jsx:206-220`. On mount the editor asks `/monitor/active` for a `generate_cover_letter` run whose `scope_key` is `cl:{id}`, and if one is live re-enters the regenerating state (its `run_id` into `regenRun`, `regening` true). That is all the existing watcher needs: the Regenerate button shows its spinner again and the letter reloads itself when the run lands, instead of the text changing under the reader with no notice. Same pattern the Resumes shelf uses for `tailor_resume`.

### R4-T2B-03 03· P2 · A failed load poisons the warm-start cache, so the *next* visit opens by asserting "0 bases · 0 tailored copies"
...
**Status**: fixed — `hooks.js:84-107` plus the three call sites. `useWarm` takes a fourth argument, `ok`: a settled-but-FAILED load neither writes `localStorage` nor hands back a snapshot, so a 500 can no longer poison the cache with zeroes for the *next* visit, and the header renders an em dash instead of asserting `0` beside its own error message. Threaded through at `Resumes.jsx:88-90`, `:149`; `CoverLetters.jsx:249-250`, `:253`; `Stats.jsx:377-386`. The same argument fixes the other three screens' count lines (R4-T2A-08).

### R4-T2B-04 04· P3 · Stats does not hold together at 900 px: the LLM-cost period pills overflow their card and the cost table collapses to ellipses
...
**Status**: fixed in part; the rest deferred (v2.1) — `Stats.jsx:90`, `:584`. The two text columns of the LLM-cost head now carry `minWidth:0` and an 8 px right pad, so `PURPOSE`/`MODEL` no longer run together as `PURPOSEMODEL` (which happened at 1024 as well as 900), and the strip reads `--label-case`/`--label-tracking-strip` like every other caps label. The 900-px work proper — letting the period-pill row wrap and reflowing the five-tile KPI strip — is left: the LLM-cost card is a fixed 300 px grid cell, so wrapping its header needs a height decision, and 900 is outside this round's width brief.

### R4-T2B-05 05· P3 · The résumé editor's ⋯ menu advertises keyboard shortcuts (`c`, `e`, `a`) that nothing implements
...
**Status**: fixed by dropping the hints — `ResumeEditor.jsx:554-562`. The three `hint="c"/"e"/"a"` slots are gone; the sibling items' prose hints stay. Binding them is **not** a one-liner each, which is why the other branch was not taken: the resume editor is a page of text fields and contenteditable bullets, so a bare single-letter key needs a target / `isContentEditable` / modifier guard first (the Feed can bind `f/j/s/x/e/o` precisely because its list pane has no text entry). If the shortcuts are wanted later, that guard is the work, not the three handlers.

### R4-T2B-06 06· P3 · Persona and Settings show nothing but the nav rail while loading; the other three screens hold their chrome
...
**Status**: fixed — `Settings.jsx:490-528` and `Persona.jsx:297-333`. Both screens hoist their header into a `header` const and render it from the loading branch as well, so the page title, subtitle and (Settings) the search field, (Persona) the Saved mark and Import button are painted from the first frame; only the body waits. The subtitle holds its box with NBSP until the data lands, so nothing jumps when it does, and the settled frame is unchanged.

### R4-T2B-07 07· P3 · Keyboard focus is effectively invisible on the dark nav rail
...
**Status**: fixed — `theme.css:166-168` (and the dark twin at `:388-390`), `:1153-1157`. New `--rail-focus` token, `var(--rail-accent)` in both base blocks so every palette resolves it to its own rail accent, and a new rule `.jn-v2 .v2-navdark:focus-visible { outline:2px solid var(--rail-focus); outline-offset:-2px }`. The existing ring rule matches `[tabindex="0"]`, which real `<a>` elements never carry — which is exactly why the rail's ten first Tab stops fell through to the UA hairline at ~1.1:1. Every rail control already carries `v2-navdark`, so the nine nav items, the Classic-UI link and the health row are all covered in one rule.

### R4-T2B-08 08· P3 · The cover-letter preview's failure line claims "Showing the previous version" when there is no previous version
...
**Status**: fixed — `CoverLetterEditor.jsx:537-540` (the `pdfErr` line). The failure line now reads "Preview failed — the PDF could not be rendered." unless `pdfUrl` is actually non-null, in which case it keeps the honest "Showing the previous version". Same distinction the resume editor already draws.

### R4-T2B-09 09· P3 · A cover letter with no linked job is filed under "Archived · from rejected applications & skipped jobs"
...
**Status**: fixed — `CoverLetters.jsx:239-242`. `isActive` now treats "no stage **and** no job status" as a draft rather than falling through to the archived band: `c.stage ? c.stage !== 'rejected' : (c.job_status ? LIVE_JOB.includes(c.job_status) : true)`. A letter created with no `job_id`, and one whose job row the cleanup scheduler later removed, both stay in the live list — where the row's own `Draft` tag already described them correctly.

### R4-T2B-10 10· P3 · Stats' empty state leaves two cards blank with no line of explanation
...
**Status**: fixed — `Stats.jsx:528`, `:654`. Score distribution says "No scored jobs yet." and Schedules says "No scheduled jobs." when their lists come back empty, matching the two neighbours that already did ("No LLM calls in this period.", "No runs yet.").

### R4-T2B-11 11· P4 · A tailor started from the résumé editor becomes invisible if you reload that editor
...
**Status**: deferred (v2.1) — the lesser sibling of R4-T2B-02, and the one that needs more than a probe: `pendingRef` is a queue with per-run toast copy, so seeding it from `/monitor/active` means reconstructing entries the editor never created. The shelf, which is where a finished copy actually appears, already resumes correctly.

### R4-T2B-12 12· P4 · Rail badge counts never refresh in a second tab
...
**Status**: deferred (v2.1) — same cross-tab question as R4-T2A-06; a badge, and a design call about which channel (a `storage` ping mirroring `jn:counts-changed`, or accepting the divergence).

### R4-T2B-13 13· P4 · The theme-key migration leaves an invalid value in `jobnavigator_theme` for ever
...
**Status**: deferred (v2.1) — harmless today by the finding's own account (both readers validate and fall back); repairing storage on read is a small change but touches the boot script in `index.html` as well as `theme.js`, and this round did not otherwise open the theme store.
