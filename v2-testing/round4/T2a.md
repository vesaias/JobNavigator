...
### R4-T2A-01 01· P2 · Job Feed · widths 1024 and 900 — the detail pane clips its own header actions
...
**Status**: fixed — `JobFeed.jsx:957-963` (list) and `:1114` (detail). The list column was `width:472, flex:0 0 472px`, so every pixel lost below 1440 came out of the detail pane; it is now `flex:'0 1 472px', minWidth:340` and the detail `flex:'1 0 420px'` (420 is what its three header controls need). **1440 is pixel-identical**: the two flex bases sum to 892 against a 1234 px pane, so no shrink runs and the columns are 472/762 exactly as before. At 1024 the list yields 74 px and the detail gets its full 420, so `Open` / `Tailor resume` / the row menu are all reachable. 900 is improved but still clips with the rail expanded (694 px pane vs a 760 px floor) — out of scope this round, see R4-T2A-03.

### R4-T2A-02 02· P2 · Applications · loading — no reserved shell, the whole screen is blank
...
**Status**: fixed — `Applications.jsx:140-147`, the removed early return at old `:306`, `:395`, `:430`. The `if (!loaded) return <div/>` is gone; the title, count line, toolbar, the four stage heads and "Select an application." are always mounted and only the rows wait, matching the other three screens. The count line and the stage counts hold their box with NBSP while the request is in flight (never a placeholder `0`), and say an em dash if it fails.

### R4-T2A-03 03· P2 · Applications · width 900 — the detail pane collapses into overlapping controls
...
**Status**: deferred (v2.1) — 900 px. Applications keeps its own fixed 472 px list column; the Feed's fix (R4-T2A-01) was scoped to the pane that actually loses its controls. This round's brief fixes 1024 and leaves 900.

### R4-T2A-04 04· P3 · Companies · width 900 — the sticky actions cell covers the Status pill
...
**Status**: deferred (v2.1) — 900 px only, and `Companies.jsx:361` already names 1024 as the design floor. Clean at 1024 and 1440.

### R4-T2A-05 05· P3 · Searches, Companies, Applications — an outside click that dismisses a menu also fires the control underneath
...
**Status**: fixed at the primitive — `ui.jsx:661-682` gives `Menu` an `onDismiss` prop that mounts exactly the fixed transparent sheet the Feed's `Drop` already drew (one z-step under the panel, taken from the panel's own `style.zIndex`), and the six menus that lacked one now pass it: `Companies.jsx:437` (Sort) and `:532` (row menu), `Searches.jsx:699` (card menu), `Applications.jsx:341` (Company filter), `:367` (Sort) and `:510` (detail menu). The dismissing click is swallowed by the sheet, so it no longer opens the drawer/row/editor underneath. The Feed's own `Drop` is left as it is — it is the reference implementation and moving it would risk the pixel gate for no behavioural gain.

### R4-T2A-06 06· P3 · all four screens — two tabs never converge
...
**Status**: deferred (v2.1) — cross-tab convergence is a design decision (a `storage` ping or BroadcastChannel), not a defect fix; see R4-T2B-12 for the rail-badge half of the same question.

### R4-T2A-07 07· P3 · Searches — a Test run does not survive a reload and cannot be recovered
...
**Status**: fixed, frontend half — `Searches.jsx:68-90` (the sessionStorage record), `:537-566` (`landTest`/`settleTest`/`pollTest` plus the mount-time resume), `:424`, `:747`. A Test now parks itself in `sessionStorage` keyed by the search: its `run_id` while the backend is still working, its outcome once it lands, both with a 10-minute TTL. On mount the screen picks it back up — the spinner and the poll if the run is still live, the modal if the result already arrived — and closing the modal (button or Escape) clears the record so it is not restored again. **The backend half is the better fix and is NOT done**: `POST /searches/{id}/test` should register with `job_monitor` (`routes_searches.py:186-224` runs the keyword modes inline and the slow ones through a bare `asyncio.create_task` into a module-level dict), which would let `/monitor/active` resume a Test the way it already resumes `search_run` and `company_scrape`. Backend call, not taken here.

### R4-T2A-08 08· P3 · all four screens — the header count line reports 0 after a failed load
...
**Status**: fixed on all four — the count line now has three states, never an accidental `0`: NBSP while the request is in flight, an em dash when it failed, numbers only when they are real. `hooks.js:74-107` gained a fourth `ok` argument to `useWarm` (a failed load neither writes the cache nor hands back a snapshot) and exports `DASH`. Sites: `JobFeed.jsx:271-277`, `:855`, `:968`; `Searches.jsx:458-467`; `Companies.jsx:257-262` (which also blanks the four tier-pill counts, since they already fell back to NBSP on a null `warm`); `Applications.jsx:140-147`, `:395`.

### R4-T2A-09 09· P3 · shell · backend down — the rail health dot stays green
...
**Status**: fixed — `V2App.jsx:71-74`, `:128-133`, `:149-172`, `:244`. The health pair now distinguishes "the server answered" from "we could not reach it": a rejection carrying a `response` (500, 401 ...) belongs to the screen behind the rail, but a rejection with **no** response at all — and only when *both* requests fail that way — sets `down`. The dot then paints `--bad` and the line reads **Backend unreachable**, with a tooltip saying nothing below it is live. Green is now reserved for a backend that actually answered.

### R4-T2A-10 10· P3 · Companies drawer — the résumé helper contradicts "Score new jobs automatically: Off"
...
**Status**: deferred (v2.1) — copy fix in the Companies drawer, no interaction with this round's scope.

### R4-T2A-11 11· P3 · Job Feed — infinite scroll silently drops rows; some jobs are unreachable
...
**Status**: fixed client-side, and the real repair named — `JobFeed.jsx:321-351`. The client keeps its dedupe by id, but no longer *stops* on a page of duplicates: `loadMore` now keeps asking for the next offset (up to six pages) until at least one new row lands, so a scroll cannot stall and every row stays reachable. Chosen because **the API has no secondary sort key to ask for** — `sort_by` is a four-value enum and `routes_jobs.py:122-129` emits a single `order_by` per branch, so `id` cannot be added from the request. The proper fix is one line of backend (append `Job.id` to all four `order_by` branches), which makes LIMIT/OFFSET deterministic and lets this loop go back to a single page; flagged for the backend lane, not taken here.

### R4-T2A-12 12· P3 · all screens — `⧉` (U+29C9) renders as a missing-glyph box
...
**Status**: fixed — a new `CopyGlyph` primitive at `ui.jsx:903-920` (two sheets, the back one an L so no line shows through the front, `currentColor`, 12 px) replaces the character at all five sites: `Applications.jsx:542` (prep pill) and `:677` (Copy to clipboard), `JobFeed.jsx:1444` (Copy with tracked links), `Searches.jsx:702` (Duplicate), `ConfirmDialog.jsx:40` (`PromptDialog`'s Copy). Being drawn rather than typeset, it no longer depends on the reader's machine having a font that covers U+29C9.

### R4-T2A-13 13· P4 · Companies — the Add-company modal throws typed input away on Escape
...
**Status**: deferred (v2.1) — the Add-company modal needs the dirty-tracking plus `ConfirmDialog` wiring the drawer and the Log-application modal already have; it is a real change, not a one-liner, and the brief scoped 13/14/15 to trivial fixes only.

### R4-T2A-14 14· P4 · Applications — Escape does not close the "+ Add interview" form
...
**Status**: fixed — `Applications.jsx:134`. `setIntForm(false)` joins the same `useEscape` callback that already closes the filter menus, the prep modal, the interview *edit* form and the Log modal, so the add form now behaves like the edit form that advertises "Escape cancels".

### R4-T2A-15 15· P4 · Companies — the row `⋯` menu stays open behind the delete confirm
...
**Status**: fixed — `Companies.jsx:318-324`, `:537`. The row menu is cleared **before** the confirm goes up (`setMenuId(null); deleteCompany(c)`), the way the Feed already does it, so it no longer sits under the scrim as clutter nor stays open when the confirm is cancelled.

### R4-T2A-16 16· P4 · Searches, Companies — the error headline and its detail line are the same sentence
...
**Status**: deferred (v2.1) — error copy, no interaction with this round's scope.

### R4-T2A-17 17· P4 · Searches, Applications — raw internal error text is shown verbatim
...
**Status**: deferred (v2.1) — needs a truncation/summarisation rule for `last_error`, which is a copy decision.

### R4-T2A-18 18· P4 · Job Feed, Applications, Companies — popovers open on a half pixel
...
**Status**: deferred (v2.1) — the popover pixel snap is the same class as D-19/RES-32 and belongs with them; `Drop` and `Menu` would both need `useSnapTop`'s treatment.

### R4-T2A-19 19· P4 · Job Feed — Enter does nothing
...
**Status**: deferred (v2.1) — nothing is broken (Enter is simply unassigned); binding it is a shortcut-design call.
