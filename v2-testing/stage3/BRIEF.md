...
## Environment
- Stack is up. Backend hot-reloads Python edits (uvicorn --reload). Frontend is a static bundle built in Docker — **do NOT run `docker compose build`** (other agents share the stack; I rebuild once after the wave). JSX fixes are therefore "fixed in source, unverified — rebuild pending".
...
## Fixing vs logging
- Fix in source only if **unambiguous and contained (< 50 lines, one obviously correct answer)**. Backend fixes: verify live after hot-reload. JSX fixes: note "fixed in source, rebuild pending". Never widen scope.
