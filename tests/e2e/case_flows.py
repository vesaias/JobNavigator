...
the manual flow passes (A: sources, B: documents).
    ...
    try:
        with h.browser() as b:
            ...
            try:
                ...
                before = (r or {}).get('counts', {}).get('/searches')
                ...
                for _ in range(20):
                    rr = C.rail(pg)
                    if str((rr or {}).get('counts', {}).get('/searches')) == str(len(_searches())):
                        agreed = True
                        break
                    pg.wait_for_timeout(150)
                c.check('the rail badge refreshes after the delete (jn:counts-changed)', agreed,
                        f'rail={(C.rail(pg) or {}).get("counts", {}).get("/searches")} api={len(_searches())}')
            finally:
                pg.context.close()
    finally:
        if ok:
