...
ROUTES = [
    ...
    ('/resumes', 'Résumés'),
    ('/cover-letters', None),      # heading text varies with the shelf state
    ('/persona', 'Persona'),
    ('/stats', 'Stats'),
    ('/settings', 'Settings'),
]
...
def _error_state(c):
    ...
    stubs = [
        ('/feed', re.compile(r'/api/jobs(\?|$)')),
        ('/searches', re.compile(r'/api/searches(\?|$)')),
        ('/companies', re.compile(r'/api/companies(\?|$)')),
        ('/applications', re.compile(r'/api/applications(\?|$)')),
        ('/persona', re.compile(r'/api/persona(\?|$)')),
