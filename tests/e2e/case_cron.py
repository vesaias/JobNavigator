...
Expectations come from the checked table at the bottom of frontend/src/time.js.
"""
...
LINE_JS = """(label) => {
  ...
  if (!inp) return null;
  // CronBox wraps its field + preset picker in a flex-column div, with the
  // helper line (the description/next-fire text) as that div's last child.
  // Input only wraps the <input> in a span when it has an adornment (secrets'
  // show/hide link) — cron fields don't, so walk up to the nearest
  // flex-column ancestor instead of assuming a fixed span/div hop count.
  let col = inp.parentElement;
  while (col && getComputedStyle(col).flexDirection !== 'column') col = col.parentElement;
  if (!col || !col.lastElementChild) return null;
    ...
    with h.browser() as b:
        ...
        try:
            ...
            if pg.locator(sel).count() == 0:
                c.skip(f'no cron field labelled {ROW!r} on /settings')
