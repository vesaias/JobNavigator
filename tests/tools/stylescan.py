"""D1 static scan: classify every inline style object in the design-system JSX by role.

Usage: py tests/tools/stylescan.py [--out tests/artifacts/scan.md] [--json path]
...
# Same scanned set as stylelint.py: frontend/src/*.jsx + frontend/src/screens/*.jsx.
# classic/ (the Tailwind v1 interface), design-base/ (git-ignored /ui gallery +
# /toasts lab) and __tests__/ are deliberately not scanned — design-base renders
# primitives on purpose, so its signatures are not screen drift.
ROOT = os.path.join(os.path.dirname(__file__), "..", "..", "frontend", "src")
DIRS = [ROOT, os.path.join(ROOT, "screens")]
...
def main():
    out = "tests/artifacts/scan.md"; js = None
    a = sys.argv[1:]
    ...
    for d in DIRS:
        ...
        for fn in sorted(os.listdir(d)):
            ...
            for line, tag, cls, st in find_styles(text):
                ...
                groups[r][(signature(st), next((c for c in HOVER_CLASSES if c in cls), ""))].append(f"{label}:{line}")
    os.makedirs(os.path.dirname(out), exist_ok=True)
    o = io.StringIO(); w = o.write
    w("# D1 static scan — inline style objects by role\n\n")
    w(f"{total} `style={{{{…}}}}` objects across `frontend/src/*.jsx` + `frontend/src/screens/*.jsx`. Roles are heuristic; the `unclassified` list is for manual triage. Each row = one distinct signature (design-relevant keys only) + hover class; sites are `file:line`.\n\n")
