...
Copy out with: docker compose cp backend:/tmp/v2t/shots/<stage> tests/artifacts/design/<stage>
...
ROUTES = ['/feed', '/searches', '/companies', '/applications', '/resumes', '/cover-letters', '/persona', '/stats', '/settings', '/toasts']
if bases: ROUTES.append(f"/resumes/{bases[0]['id']}")
if withjob: ROUTES.append(f"/resumes/{withjob['id']}")
if cls: ROUTES.append(f"/cover-letters/{cls[0]['id']}")
index = {}
with browser() as b:
