...
Usage: py tests/tools/stylelint.py [--strict]
Checks frontend/src/*.jsx and frontend/src/screens/*.jsx (except ui.jsx) plus theme.css for raw
colours, font/radius/shadow literals outside ui.jsx, hover classes on non-primitives, and
...
# The design system is src/*.jsx + src/screens/*.jsx. classic/ is the untouched
# Tailwind v1 interface, design-base/ is the git-ignored lab pages and __tests__/
# is Vitest — none of them are screens, so none are scanned; only ui.jsx (the
# primitive layer, the one file allowed to carry literals) needs skipping.
...
def lint_jsx(fn, text):
    ...
    for i, line in enumerate(lines, 1):
        ...
        # `&` guard: an HTML numeric entity (`&#129517;`) is not a colour literal.
...
def main():
    ...
    for d in DIRS:
        ...
        for fn in sorted(os.listdir(d)):
            ...
            for i, kind, ok, snippet in lint_jsx(fn, io.open(os.path.join(d, fn), encoding="utf-8").read()):
                (allowed if ok else findings).append((label, i, kind, snippet))
