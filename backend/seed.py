...
DEFAULT_SETTINGS = {
    ...
    "persona_tailor_prompt": ("Tailor a FOCUSED resume from this rich candidate profile, targeted at the job description below.\n\nThe candidate profile is a deep pool — most roles have many bullets. Your job is to SELECT the best ones aligned with the JD and reformulate them tightly.\n\nRules:\n- Rewrite the summary to target this specific role (2-4 sentences, lead with the most relevant strength)\n- For each experience entry: SELECT only 3-5 bullets (max 6 for the most senior/recent role). Pick the bullets that best match JD keywords and required skills; drop the rest\n- Reformulate each selected bullet to use the JD's exact vocabulary where possible\n- For each experience entry, optionally include 1-2 suggested_bullets[] (STAR-format wins worth highlighting that didn't make the main cut)\n- Reorder skills to prioritize JD-relevant ones first; cap at 6 categories\n- Do NOT invent new experience, skills, or facts. Only reframe existing content\n- NEVER add skills the candidate does not have\n- Preserve all company names, titles, dates, locations, education exactly\n- Do NOT use em-dashes or unicode special characters. Use regular hyphens (-) and ASCII only\n- Preserve **bold** formatting (double asterisks). For reformulated bullets, wrap the strongest metric in **bold** (one per bullet)\n- VERIFICATION: every selected/reformulated bullet must be traceable to the candidate profile. If you cannot trace it, drop it\n\nCandidate Profile:\n{resume_json}\n\nJob Description:\n{job_description}\n\nReturn ONLY this JSON:\n{\"summary\": \"rewritten summary\", \"experience\": [{\"company\": \"unchanged\", \"title\": \"unchanged\", \"location\": \"unchanged\", \"date\": \"unchanged\", \"description\": \"unchanged or null\", \"bullets\": [\"selected + reformulated bullet\"], \"suggested_bullets\": [\"optional STAR-format extra\"]}], \"skills\": {\"reordered label\": \"reordered value\"}}", "Editable Persona tailoring LLM prompt template — used when base_resume_id='persona' to constrain bullet selection from the rich pool"),
