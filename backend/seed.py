...
DEFAULT_SETTINGS = {
    ...
    "speedyapply_enabled": ("true", "Enable the daily SpeedyApply college-jobs workflow"),
    "speedyapply_cron": ("0 9 * * *", "Daily SpeedyApply cron (min hour day month dow)"),
    "speedyapply_secondary_cron": ("30 20 * * *", "Additional daily SpeedyApply cron (min hour day month dow)"),
    "speedyapply_timezone": ("Asia/Shanghai", "IANA timezone for the SpeedyApply daily cron"),
    "speedyapply_feeds": (json.dumps(["intern_usa", "new_grad_usa"]), "Enabled SpeedyApply feed IDs"),
    "speedyapply_max_age_days": ("1", "Only prepare postings at most this many days old"),
    "speedyapply_max_jobs_per_run": ("25", "Maximum jobs imported/prepared per daily run"),
    "speedyapply_auto_tailor": ("true", "Automatically tailor a resume for each imported job"),
    "speedyapply_resume_id": ("", "Base Resume UUID or 'persona' for SpeedyApply tailoring"),
    "job_feeds_enabled": ("true", "Enable realtime aggregate job-feed monitoring"),
    "job_feeds_interval_minutes": ("5", "How often GitHub aggregate feeds are checked"),
    "job_feeds_worker_interval_minutes": ("1", "How often pending resume packets are prepared"),
    "job_feeds_sources": (json.dumps([
        "speedyapply_intern_usa", "speedyapply_new_grad_usa",
        "vansh_summer_2027", "vansh_new_grad_2027",
    ]), "Enabled aggregate job feed IDs"),
    "job_feeds_backfill_days": ("7", "Initial and retry lookback window for aggregate postings"),
    "job_feeds_max_jobs_per_poll": ("25", "Maximum eligible postings enqueued per feed poll"),
    "job_feeds_role_families": (json.dumps(["swe", "ai", "data"]), "Role families prepared automatically"),
    "job_feeds_work_auth": ("f1_opt_future_sponsorship", "Work authorization filter profile"),
    "job_feeds_resume_source": ("persona", "Resume source for automatic feed tailoring"),
    "job_feeds_auto_tailor": ("true", "Automatically prepare packets for eligible feed jobs"),
    "job_feeds_score_after_tailor": ("false", "Never score resumes created by the feed workflow"),
    "job_feeds_notification_email": ("", "Notification recipient; empty uses Persona contact.email"),
    "job_feeds_email_enabled": ("true", "Send detected and terminal Gmail notifications"),
    "job_feeds_artifact_dir": ("application-packets", "Project-local application packet directory"),
    "job_feeds_persona_prompt": ("Tailor a one-page ATS resume from the candidate profile for the job description.\n\nHard rules:\n- Use only facts, skills, employers, projects, dates, titles, locations, and numeric claims already present in the candidate profile.\n- Never invent or infer a missing skill or metric. A JD keyword may appear only when the profile already supports the same skill or fact.\n- Keep every experience entry in the same order and preserve company, title, date, location, and education exactly.\n- Select 2-4 of the strongest existing bullets per role. Reformulate minimally with supported JD vocabulary.\n- Keep standard ATS section language and enough content to fit one US Letter page.\n- Do not return suggested_bullets or speculative gap fillers.\n- Reorder existing skills by relevance; do not add skills.\n\nCandidate Profile:\n{resume_json}\n\nJob Description:\n{job_description}\n\nReturn ONLY this JSON:\n{\"summary\": \"targeted factual summary\", \"experience\": [{\"company\": \"unchanged\", \"title\": \"unchanged\", \"location\": \"unchanged\", \"date\": \"unchanged\", \"description\": null, \"bullets\": [\"selected or minimally reformulated existing bullet\"]}], \"skills\": {\"existing category\": \"reordered existing skills\"}}", "Fact-constrained Persona prompt for automatic application packets"),
...
END $$;""",
    ...
    updated_at TIMESTAMPTZ DEFAULT NOW()
        ...
        """CREATE TABLE IF NOT EXISTS application_queue_items (
            ...
            base_resume_key VARCHAR,
            source_feed VARCHAR,
            application_url VARCHAR,
            status VARCHAR NOT NULL DEFAULT 'pending_description',
        ...
        "CREATE INDEX IF NOT EXISTS ix_application_queue_status ON application_queue_items(status)",
        "CREATE INDEX IF NOT EXISTS ix_application_queue_created_at ON application_queue_items(created_at)",
        "ALTER TABLE application_queue_items ADD COLUMN IF NOT EXISTS application_url VARCHAR",
        "ALTER TABLE application_queue_items ADD COLUMN IF NOT EXISTS source_posted_at TIMESTAMPTZ",
        "ALTER TABLE application_queue_items ADD COLUMN IF NOT EXISTS priority INTEGER NOT NULL DEFAULT 10",
        "ALTER TABLE application_queue_items ADD COLUMN IF NOT EXISTS eligibility_warnings JSONB DEFAULT '[]'::jsonb",
        "ALTER TABLE application_queue_items ADD COLUMN IF NOT EXISTS artifact_dir VARCHAR",
        "ALTER TABLE application_queue_items ADD COLUMN IF NOT EXISTS detected_notified_at TIMESTAMPTZ",
        "ALTER TABLE application_queue_items ADD COLUMN IF NOT EXISTS terminal_notified_at TIMESTAMPTZ",
        """CREATE TABLE IF NOT EXISTS job_feed_checkpoints (
        ...
        """CREATE TABLE IF NOT EXISTS job_feed_postings (
            id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            ...
            CONSTRAINT uq_job_feed_posting_source_key UNIQUE (source_id, source_key)
        )""",
        "CREATE INDEX IF NOT EXISTS ix_job_feed_posting_job_id ON job_feed_postings(job_id)",
    ]
