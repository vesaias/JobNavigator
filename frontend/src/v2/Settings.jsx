  ...
  const sections = useMemo(() => {
    ...
    return [
      ...
      ['scoring', '', 'Scoring behavior', 'which résumé gets scored, how deep, and when it runs', [
        ...
        B('Max parallel jobs', 'Extra requests queue — protects the DB pool.', 'scoring_max_concurrent', { mono: true, w: '120px' }),
      ...
      ['autofill', '', 'Autofill', 'used by the extension on ATS forms', [
        B('Default answer length', 'Target length for form answers, in characters.', 'autofill_default_length', { mono: true, w: '120px' }),
      ...
      ['emailclass', '', 'Email classification', 'turns Gmail replies into application events', [
        ...
        B('Confidence threshold', '0–100 — below this, the email is flagged for manual review instead.', 'email_llm_confidence_threshold', { mono: true, w: '120px' }),
      ...
      ['scheduler', 'PIPELINE', 'Scheduler', 'intervals in minutes (0 = off) · crons are min hour day month dow, empty = off', [
        B('Scrape all companies', 'Runs every active company scrape on this interval.', 'scrape_interval_minutes', { mono: true, w: '120px' }),
        B('Email check', 'Polls Gmail for replies to your applications.', 'email_check_interval_minutes', { mono: true, w: '120px' }),
        B('Cleanup after', 'Days before unsaved postings are purged.', 'job_archive_after_days', { mono: true, w: '120px' }),
        B('Auto-reject after', 'Days of silence before an application is auto-moved to Rejected.', 'auto_reject_after_days', { mono: true, w: '120px',
        ...
        B('DB backup · cron', 'Nightly database snapshot.', 'backup_cron', { mono: true, w: '120px' }),
        B('Telegram digest · cron', 'Morning summary of new high-fit jobs.', 'digest_cron', { mono: true, w: '120px' }),
        B('H-1B refresh · cron', 'Re-imports the sponsorship dataset.', 'h1b_cron', { mono: true, w: '120px' }),
        B('Job cleanup · cron', 'Purges expired postings.', 'cleanup_cron', { mono: true, w: '120px' }),
        B('Auto-reject · cron', 'Applies the auto-reject threshold.', 'reject_cron', { mono: true, w: '120px' }),
        B('Max parallel tailors', 'Tailoring and cover-letter generation share this limit.', 'tailoring_max_concurrent', { mono: true, w: '120px' }),
      ]],
      ...
      ['notifications', 'INTEGRATIONS', 'Notifications', 'Telegram bot · digest schedule lives under Scheduler', [
        ...
        B('Chat ID', 'Your Telegram chat — get it by messaging @userinfobot.', 'telegram_chat_id', { mono: true, w: '120px' }),
        B('Score threshold', 'Only jobs scoring at or above this trigger an instant alert — everything else waits for the digest.', 'fit_score_threshold', { mono: true, w: '120px' }),
